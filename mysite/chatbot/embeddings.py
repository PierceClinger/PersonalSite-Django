...
os.environ["OPENAI_API_KEY"] = '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'

openai.api_key = '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'
...
for g1 in range(len(gfiles)): # Iterating through every document
    ...
    f = open(f"mysite\chatbot\embs{g1}.csv", "w") # Creating a csv file for storing the embeddings for your ChatBot
...
with open(f"{gfiles[g1]}", 'r', encoding="utf-8") as file: # Storing the document contents
    content += file.read()
    content +=  "\n\n"
...
df = pandas.read_csv(f"mysite\chatbot\embs{g1}.csv") # Reading the empty csv file that you created earlier for storing the embeddings
...
df.to_csv(f"mysite\chatbot\embs{g1}.csv") # Writing the data to the csv file
...
df.to_csv(f"mysite\chatbot\embs{g1}.csv", index=False) # Writing the new 'embedding' collumn to the csv file
df = pandas.read_csv(f"mysite\chatbot\embs{g1}.csv") # Reading the new csv file
embs = []
...
df.to_csv(f"mysite\chatbot\embs{g1}.csv", index=False) # Writing the final version of the csv file